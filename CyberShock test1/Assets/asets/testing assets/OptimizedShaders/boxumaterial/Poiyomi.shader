Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/20a93503a24f7d64c8139b3c37f6931a"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void initPoiAudioLink(inout PoiMods poiMods)
			{
				...
				if (AudioLinkIsAvailable())
				{
					...
					poiMods.audioLink[0] = (0.3 /*_AudioLinkSmoothingBass*/)    == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 0))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.3 /*_AudioLinkSmoothingBass*/)   ) * 15.95, 0))[0];
					poiMods.audioLink[1] = (0.3 /*_AudioLinkSmoothingLowMid*/)  == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 1))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.3 /*_AudioLinkSmoothingLowMid*/) ) * 15.95, 1))[0];
					poiMods.audioLink[2] = (0.3 /*_AudioLinkSmoothingHighMid*/) == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 2))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.3 /*_AudioLinkSmoothingHighMid*/)) * 15.95, 2))[0];
					poiMods.audioLink[3] = (0.3 /*_AudioLinkSmoothingTreble*/)  == 0 ? AudioLinkData(ALPASS_AUDIOLINK + float2(0, 3))[0] : AudioLinkData(ALPASS_FILTEREDAUDIOLINK + float2((1 - (0.3 /*_AudioLinkSmoothingTreble*/) ) * 15.95, 3))[0];
			...
			void ApplyAudioLinkDecal(in PoiMesh poiMesh, inout PoiFragData poiFragData, in PoiMods poiMods)
			{
				...
				float2 decalCenter = float4(-0.05,0.47,1,1);
				float theta = radians((270.0 /*_ALUVRotation*/) + _Time.z * (0.0 /*_ALUVRotationSpeed*/));
				float cs = cos(theta);
				float sn = sin(theta);
				uv = float2((uv.x - decalCenter.x) * cs - (uv.y - decalCenter.y) * sn + decalCenter.x, (uv.x - decalCenter.x) * sn + (uv.y - decalCenter.y) * cs + decalCenter.y);
				uv = remap(uv, float2(0, 0) - float4(0.49,0.66,1,-0.01).xz / 2 + float4(-0.05,0.47,1,1), float4(0.49,0.66,1,-0.01).yw / 2 + float4(-0.05,0.47,1,1), float2(0, 0), float2(1, 1));
				...
				audioLinkMask.b = maskVolume < (0.0 /*_ALDecalVolumeClipMin*/) || maskVolume > (0.741 /*_ALDecalVolumeClipMax*/) ? 0.0 : audioLinkMask.b;
				...
				{
					float volumeth = (0.492 /*_ALDecalShapeClipVolumeWidth*/);
					if ((0.0 /*_ALDecalVolumeStep*/) != 0.0) audioLinkMask.b = frac(maskY * (0.0 /*_ALDecalVolumeStep*/)) > volumeth ? 0.0 : audioLinkMask.b;
					float bandwidth = (0.0 /*_ALDecalUVMode*/) == 1 ? (0.0 /*_ALDecalShapeClipBandWidth*/) / aluv.y : (0.0 /*_ALDecalShapeClipBandWidth*/);
				...
				float audioVal = audioTexture.b * (0.5 /*_ALDecalVolume*/) * lerp((5.0 /*_ALDecalBaseBoost*/), (2.5 /*_ALDecalTrebleBoost*/), audioLinkMask.g);
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMapUV*/)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.6789893,0.8705882,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed*/)).rgb * lerp(1, poiFragData.baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.6789893,0.8705882,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, (1.0 /*_EmissionBaseColorAsMap*/)).rgb * poiThemeColor(poiMods, float4(0,0.6789893,0.8705882,1).rgb, (0.0 /*_EmissionColorThemeIndex*/));
				#endif
				...
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength0, (0.0 /*_AudioLinkEmission0CenterOutSize*/), (0.0 /*_AudioLinkEmission0CenterOutBand*/), float4(0,1,0,0), (1.0 /*_EmissionAL0Enabled*/), (1.0 /*_AudioLinkEmission0CenterOutDuration*/));
